# This SQL is used for conducting evaluation of profiles for accepting functional cookies (#2 #4 #8).


-- e6.1 number of cookies per webpage
SELECT
  alias,
  AVG(ct) avg,
  STDDEV(ct) stdev,
...
FROM (
  ...
  WHERE
    (cookies.fc_getAlias(browser_id)='#2'
      OR cookies.fc_getAlias(browser_id)='#4'
...
SELECT
  '2_4' compare,
  AVG(name.sim_2_4) avg,
  STDDEV(name.sim_2_4) std,
  MIN(name.sim_2_4) min,
  MAX(name.sim_2_4) max
FROM
...
select
  '2_4_8',
  AVG(name.sim_2_4_8) avg,
  STDDEV(name.sim_2_4_8) std,
  MIN(name.sim_2_4_8) min,
  MAX(name.sim_2_4_8) max
FROM
  cookies.sim_cookies_per_visit
 order by compare;
